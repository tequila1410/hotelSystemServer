...
function getAll(callback) {
    db.query('SELECT *, c.name as categoryName, c.price*r.countSeats as price from room r, category c where idCategory = idCategories;',
...
function findByNumber(number, callback) {
    db.query('SELECT *, c.name as categoryName, c.price*r.countSeats as price from room r, category c where idCategory = idCategories and number = ?', [number],
        function (err, room, fields) {
            if (err) {
                callback(err);
                return
...
function findByCategoryAndSeats(idCategory, countSeats, callback) {
    db.query('SELECT r.idRoom, r.number, c.price * r.countSeats as price ' +
        ...
        'WHERE isEmpty = 1 AND r.idCategory = ? AND r.idCategory = c.idCategories AND r.countSeats = ?', [idCategory, countSeats],
        function (err, room, fields) {
            if (err) {
                callback(err);
                return
